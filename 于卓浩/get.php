...
$e = 0;
$stuID = (int) $stuID;
if(file_exists("./store/{$stuID}.json")){
    echo file_get_contents("./store/{$stuID}.json");
    exit;
}
...
$schoolCode = "10611";
if(!isset($_GET["pass"])) {die("请在get参数中加pass={pass明文}");}
$pass = $_GET["pass"];
...
system("python ./server/ics-convert.py {$stuID}.ics ./store/{$stuID}.json");
echo file_get_contents("./store/{$stuID}.json");