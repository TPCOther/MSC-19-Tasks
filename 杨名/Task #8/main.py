...
def pwd_md5(sid, pwd): # md5加密 明文->密文
    schoolcode = '10611'

    def get_md5(str): # 返回MD5
        return hashlib.md5(str.encode("utf-8")).hexdigest()
    return get_md5(sid + get_md5(pwd)[0:30].upper()+schoolcode)[0:30].upper()


def stu_login(index_url, stu_id, stu_pwd): # 利用cookie登录教务网
    ...
    opener.open(urllibre.Request(url=index_url, data=postdata))
    return opener


def cat_crs_tab_webpage(login_opener, semester, crs_tab_url): # 查询学期课表
    postdata = urllibpar.urlencode({
        'Sel_XNXQ': semester,
        'rad': 'on',
        'px': '1'
    ...
    page_str = res.read().decode('gb2312') # 宋体
    return page_str


def get_first_week_timeline(first_day): # 获取第一周作息表
    time_line = []
    first_day_timeline = [
    ...
    for one_crs in crs_dicts:
        for lesson_num in one_crs['节数']:
            ...
            for week_num in one_crs['课程周次']:
                ...
                tmp_event.add(
                    'location', str(one_crs['上课地点'])+' '+str(one_crs['授课老师'])+' 第'+str(week_num)+'周')
...
if __name__ == "__main__":
    ...
    # 捕获课表页面
    crs_tab_page_str = cat_crs_tab_webpage(login_opener, current_semester, crstable_url)
    # 从课表页面获取课程信息列表
